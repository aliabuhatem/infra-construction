...
// Only verified links — Twitter & YouTube removed (no official link provided)
const social = [
  {
    label: "LinkedIn",
  ...
  {
    label: "Facebook",
    href: "https://www.facebook.com/share/1DznurcqWH/",
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    ...
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 13.065L.726 4.5h22.548L12 13.065zM12 15.435L.5 6.84V19.5h23V6.84l-11.5 8.595z" />
      </svg>
    ),
  },
];
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-4 gap-10">
          ...
          <div className="flex gap-3">
            {social.map((s) => (
              <a
                ...
                aria-label={s.label}
                className="w-9 h-9 border border-white/20 flex items-center justify-center hover:bg-[#4FC3B7] hover:border-[#4FC3B7] transition"
              >
                {s.icon}
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-2 mb-5">
            {offices.map((o) => (
              <li key={o.city} className="text-sm" style={{ fontFamily: B }}>
          ...
          <div className="space-y-2 pt-4 border-t border-white/10" style={{ fontFamily: B }}>
            <a href={`mailto:${CONTACT.email}`} className="block text-sm text-white/70 hover:text-[#4FC3B7]">
              ✉ {CONTACT.email}
            </a>
            <a href={`tel:${CONTACT.phone}`} className="block text-sm text-white/70 hover:text-[#4FC3B7]">
