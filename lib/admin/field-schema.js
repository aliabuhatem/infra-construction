...
const FIELD_DEFAULTS = {
  ...
  // The store carries two parallel families — heading* and headline*. Both are
  // real and in use, so both are described rather than normalised away, which
  // would mean renaming live fields.
  heading:       { type: T.TEXT, group: "content", order: 14, max: 200, label: "Heading" },
  headingLine1:  { type: T.TEXT, group: "content", order: 15, max: 200, label: "Heading line 1" },
  headingLine2:  { type: T.TEXT, group: "content", order: 16, max: 200, label: "Heading line 2" },
  headingLine3:  { type: T.TEXT, group: "content", order: 17, max: 200, label: "Heading line 3" },
  headlineLine1: { type: T.TEXT, group: "content", order: 18, max: 200, label: "Headline line 1" },
  headlineLine2: { type: T.TEXT, group: "content", order: 19, max: 200, label: "Headline line 2" },
  headlineLine3: { type: T.TEXT, group: "content", order: 20, max: 200, label: "Headline line 3",
                   hidden: true,
  ...
  bio:   { type: T.TEXTAREA, group: "content", order: 24, label: "Biography" },
  desc:  { type: T.TEXTAREA, group: "content", order: 25, label: "Description" },
  short: { type: T.TEXT, group: "content", order: 26, max: 200, label: "Short text" },
  // Comma-separated in the store (see about_dept_* items) — splitList semantics.
  items: { type: T.LIST, group: "content", order: 55, label: "Items",
           lineRule: "One line or comma-separated entry = one item.",
           noun: "item", nounPlural: "items" },
  ctaTitle: { type: T.TEXT, group: "content", order: 60, max: 200, label: "Call-to-action title" },
  ctaBody:  { type: T.TEXTAREA, group: "content", order: 61, label: "Call-to-action body" },
  ...
  year:     { type: T.TEXT, group: "metadata", order: 30, max: 20, label: "Year",
              pattern: /^\d{4}(\s*[–-]\s*\d{4})?$/,
              patternHint: 'A year, or a range such as "2019 – 2022".' },
  duration: { type: T.TEXT, group: "metadata", order: 31, max: 60, label: "Duration" },
  client:   { type: T.TEXT, group: "metadata", order: 32, max: 160, label: "Client" },
  principalContractor: { type: T.TEXT, group: "metadata", order: 33, max: 160, label: "Principal contractor" },
  license:  { type: T.TEXT, group: "metadata", order: 34, max: 120, label: "Licence" },
  readTime: { type: T.TEXT, group: "metadata", order: 35, max: 30, label: "Read time" },
  code:     { type: T.TEXT, group: "metadata", order: 36, max: 40, label: "Code" },
  label:      { type: T.TEXT, group: "metadata", order: 40, max: 120, label: "Label" },
  badgeLabel: { type: T.TEXT, group: "metadata", order: 41, max: 60,  label: "Badge label" },
  stat3Value: { type: T.TEXT, group: "metadata", order: 27, max: 20, label: "Stat 3 value" },
  stat1Label: { type: T.TEXT, group: "metadata", order: 42, max: 60, label: "Stat 1 label" },
  stat2Label: { type: T.TEXT, group: "metadata", order: 43, max: 60, label: "Stat 2 label" },
  stat3Label: { type: T.TEXT, group: "metadata", order: 44, max: 60, label: "Stat 3 label" },
  stat4Label: { type: T.TEXT, group: "metadata", order: 45, max: 60, label: "Stat 4 label" },

  // ── links ───────────────────────────────────────────────────────────────
  link:        { type: T.URL,  group: "links", order: 5, label: "Link" },
  linkText:    { type: T.TEXT, group: "links", order: 6, max: 80, label: "Link text" },
  buttonText:  { type: T.TEXT, group: "links", order: 7, max: 80, label: "Button text" },
  exploreText: { type: T.TEXT, group: "links", order: 8, max: 80, label: "Explore link text" },
...
/**
 * Field-name heuristic inherited from the panel's original isLikelyImageField().
 * It runs after the explicit tables but before value sniffing, matching the old
 * precedence exactly: a name like `icon`, `favicon` or `thumb` that the schema
 * does not list still gets the image preview and the media picker it has always
 * had, even when its value looks like something else.
 */
const LEGACY_IMAGE_KEY_RE = /image|photo|logo|icon|background|favicon|cover|thumb/i;
...
export function describeField(sectionId, fieldName, value) {
  ...
  const resolved =
    ...
    FIELD_DEFAULTS[name] ||
    (LEGACY_IMAGE_KEY_RE.test(name) ? { type: T.IMAGE, group: "media", order: 30 } : null) ||
...
/* ─────────────────────────────────────────────────────────────────────────
   SECTION GROUPING
   Which navigation group a section belongs to, and the order those groups are
   listed in. Moved here from AdminPanel.jsx so that the rules describing
   content live beside the rules describing fields, rather than inside the view.
   Behaviour is unchanged.
   ───────────────────────────────────────────────────────────────────────── */

/** Navigation groups, in the order the panel lists them. */
export const SECTION_GROUP_ORDER = Object.freeze([
  "Home Page",
  "Sectors",
  "Sectors · Subsectors",
  "Services",
...
/**
 * The navigation group for a section id, or null when no rule matches — the
 * caller supplies its own label in that case, so this module stays free of
 * presentation concerns.
 */
export function sectionGroup(name) {
  const lower = String(name || "").toLowerCase();
  if (lower.startsWith("home_") || lower === "home") return "Home Page";
  // Expertise system: svc_<slug> / sct_<slug> (+ svc_hub / sct_hub), with
  // sub_<sector>_<subsector> for the divisions inside a sector.
...
/** Known groups first in SECTION_GROUP_ORDER, then anything else as-is. */
export function sortedSectionGroups(groups) {
  const ordered = [];
  for (const g of SECTION_GROUP_ORDER) if (groups[g]) ordered.push([g, groups[g]]);
  for (const [g, v] of Object.entries(groups)) {
    if (!SECTION_GROUP_ORDER.includes(g)) ordered.push([g, v]);
  }
  return ordered;
}
