...
export function revalidateAllContent(revalidatePath) {
  ...
  // Dynamic project detail routes (/projects/<slug>). Missing until now, so an
  // edited project kept its old copy in the served HTML and only corrected
  // itself once the client-side store arrived.
  try { revalidatePath("/projects/[slug]", "page"); } catch {}
