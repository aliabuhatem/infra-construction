...
// Where uploads go is decided by whether the filesystem is writable, not by
// whether Blob credentials happen to exist.
//
// On Vercel, public/ is baked into the build output and the runtime filesystem
// is read-only outside /tmp (which is wiped between invocations), so uploads
// MUST go to Blob — writing to public/media there fails with EROFS.
//
// Everywhere else (local dev, a VPS) public/media is writable and `next dev`
// serves it from disk on every request, so uploads stay in the repo folder
// where they can be reviewed and committed. Set MEDIA_STORAGE=blob to opt into
// Blob locally, or MEDIA_STORAGE=disk to force disk (which will fail on Vercel).
function shouldUseBlobStorage() {
  const mode = String(process.env.MEDIA_STORAGE || "").toLowerCase();
 ...
 *
 * Vercel Blob returns a CDN URL that is live the moment put() resolves, so it
 * is used on Vercel. Local dev keeps writing to public/media, which is fine
 * there because `next dev` serves public/ from disk on every request — see
 * shouldUseBlobStorage() above for how the two are chosen.
 *
...
export async function uploadMediaFile(originalName, buffer, contentType = "application/octet-stream") {
  ...
  if (shouldUseBlobStorage()) {
    if (!hasBlobConfig()) {
      throw new Error(
        "This host has a read-only filesystem, so uploads must go to Vercel Blob, " +
        "but no Blob credentials were found. Connect a Blob store to the project " +
        "(providing BLOB_STORE_ID with OIDC) or set BLOB_READ_WRITE_TOKEN."
      );
    }
    let uploaded;
    try {
  ...
  // ── local / writable-filesystem path: save into public/media ────────────
