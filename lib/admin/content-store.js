...
function mergeWithDefaults(saved) {
  ...
  for (const [k, defaultFields] of Object.entries(defaultContent)) {
    if (deletedSections.has(k)) continue;
...
export async function readContentStore({ includeSiteMedia = true } = {}) {
  ...
  if (hasGithubConfig()) {
    // Use in-process cache when available (set by writeContentStore after every write).
    // This avoids a read-after-write race where GitHub's API may return the previous
    // file content on an immediate GET following a PUT.
    if (memoryCache) {
      base = memoryCache;
    } else {
...
export async function uploadMediaFile(originalName, buffer) {
  ...
  if (hasGithubConfig()) {
    const cfg = githubConfig();
    const res = await githubRequest(
      `https://api.github.com/repos/${cfg.repo}/contents/public/media/${fileName}`,
      {
        method:  "PUT",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({
  ...
  // Save directly into public/media/
